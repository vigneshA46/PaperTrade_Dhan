...
COMMON_ID = "452e46da-a1df-4fdd-8776-66c825f2aeb6"
SYMBOL = "NIFTY"
...
strategy_id = "452e46da-a1df-4fdd-8776-66c825f2aeb6"
...
def get_futures_first_candle_mark():

    today = datetime.now(IST).strftime("%Y-%m-%d")

    fut_data = dhan.intraday_minute_data(
        security_id=int(FUT_ID),
    ...
    for i in range(len(timestamps)):
        ...
        if ts.hour == 9 and ts.minute == 15:
            ...
            print(
                f"📍 FUTURES 09:15 MARK: {mark}"
            )

            return mark

    print("❌ Futures 09:15 candle not found")
...
# =========================
# FUTURES REFERENCE MARK
# =========================

futures_mark = get_futures_first_candle_mark()

print("📌 NIFTY FUTURES MARK:", futures_mark)
...
def handle_leg(
    ...
    if not state["position"] and not state["rearm_required"]:
        ...
        if name == "CE":
            ...
            if (
                close > futures_mark and
                avg > futures_mark and
                avg < close
            ):
                signal = True
...
def handle_futures_candle(candle):
...
def on_message(msg):
    ...
    if token == FUT_ID:
        ...
        if candle:
            ...
            print("📊 FUTURES CANDLE")
            print(candle)

            handle_futures_candle(candle)
...
instruments = [
    ...
    (MarketFeed.NSE_FNO, str(FUT_ID), MarketFeed.Quote)
]
